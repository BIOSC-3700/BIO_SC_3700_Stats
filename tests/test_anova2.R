...
fac <- read_csv("data/intertidal_algae.csv",
                show_col_types = FALSE)
stub <- function(raw_df, vlab = "sqrt.area",
                 glab = "height") {
  list(raw = reactive(raw_df), tidy = reactive(NULL),
       info = reactive(NULL),
...
# ---- core model ------------------------------------------------------
testServer(mod_anova2_server,
           args = list(data = stub(fac)), {
  base_inputs(session)
  ref <- summary(aov(sqrt.area ~ height * herbivores,
                     data = fac))[[1]]
  at <- anova_terms()
  ok("no problems on clean factorial",
     length(problems()) == 0)
  ...
  ok("residual df matches",
     at$df_resid == ref[4, "Df"])
  ok("design detected as balanced",
     isTRUE(is_balanced()))
  ok("cells built with 4 cells",
     nlevels(cells()$cell) == 4)

  # cell-level Tukey vs. reference one-way cell model
  cd <- cells()
  refc <- as.data.frame(
    TukeyHSD(aov(value ~ cell, data = cd))$cell)
  tk <- tukey_cells()
...
set.seed(9)
add <- expand.grid(g = c("g1", "g2", "g3"),
                   t = c("t1", "t2"),
                   r = 1:6, stringsAsFactors = FALSE)
add$y <- 10 + 3 * (add$g == "g2") + 6 * (add$g == "g3") +
  2 * (add$t == "t2") + rnorm(nrow(add), 0, 1.5)
testServer(mod_anova2_server,
           args = list(data = stub(add, "y", "g")), {
  base_inputs(session, response = "y", fac_a = "g",
              fac_b = "t")
  ok("additive data: interaction not significant",
     !interaction_sig())
  reft <- as.data.frame(
    TukeyHSD(aov(y ~ g * t, data = add),
             which = "g")$g)
  tk <- tukey_main("a")
...
testServer(mod_anova2_server,
  ...
  session$setInputs(fac_a = "herbivores",
                    fac_b = "height")
  ss_ba <- anova_terms()$table$`Sum Sq`[2]
  ok("Type I SS is order-INdependent when balanced",
     near(ss_ab, ss_ba))
})

# ---- blocking validation ---------------------------------------------
empty_cell <- fac[!(fac$height == "mid" &
                    fac$herbivores == "plus"), ]
testServer(mod_anova2_server,
           args = list(data = stub(empty_cell)), {
  base_inputs(session)
  ok("empty cell blocked",
     any(grepl("empty", problems())))
})
one_per <- fac |>
  ...
  ungroup()
testServer(mod_anova2_server,
           args = list(data = stub(one_per)), {
  base_inputs(session)
  ok("n=1 per cell blocked",
     any(grepl("at least 2", problems())))
})
testServer(mod_anova2_server,
           args = list(data = stub(fac)), {
  base_inputs(session, fac_b = "height")
  ok("duplicate column choice blocked",
     any(grepl("three different columns", problems())))
})
one_lvl <- fac
one_lvl$herbivores <- "minus"
testServer(mod_anova2_server,
           args = list(data = stub(one_lvl)), {
  base_inputs(session)
  ok("single-level factor blocked",
     any(grepl("only one level", problems())))
})
...
testServer(mod_anova2_server,
  ...
  body <- output$body
  ok("body renders",
     !is.null(body$html) && nchar(body$html) > 200)
  ...
  ok("main-effect Tukey section renders",
     grepl("Tukey HSD for g",
           output$main_effect_tukey$html))
  ok("2-level factor omitted from post-hoc",
     !grepl("Tukey HSD for t",
            output$main_effect_tukey$html))
...
testServer(mod_anova2_server,
  ...
  ok("interaction branch omits main-effect card",
     !grepl("Post-hoc: main effects", body$html))
  ok("verdict states dependence",
     grepl("depends on", output$verdict$html))
  ok("simple effects table renders",
     nchar(output$simple_table) > 100)
  ok("cell counts table renders",
     nchar(output$counts_table) > 50)
})
