...
plot_tukey <- function(
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::scale_color_manual(
      values = c(
        "Significant" = pal$accent,
        "Not significant" = pal$muted
      ),
...
plot_bar <- function(data, title = NULL, xlab = "Group", ylab = "Value") {
  data <- dplyr::filter(data, !is.na(.data$value))
  data$group <- droplevels(data$group)
  stats <- data |>
...
plot_histogram <- function(values, bins = 20, title = NULL, xlab = "Value") {
  values <- values[!is.na(values)]
...
plot_faceted_histogram <- function(
  data,
  value_col,
  facet_col,
  bins = 20,
